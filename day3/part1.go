...
func main() {
	...
	for i, line := range lines {
		rucksacks[i].compartmentA = line[:len(line)/2]
		rucksacks[i].compartmentB = line[len(line)/2:]
		for _, ch := range rucksacks[i].compartmentA {
			if strings.ContainsRune(rucksacks[i].compartmentB, ch) && rucksacks[i].common == "" {
				rucksacks[i].common = string(ch)
				idx := strings.Index(items, string(ch))
				if idx > -1 {
					sum += idx + 1
					continue
				}
